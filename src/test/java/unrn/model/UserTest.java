...
class UserTest {
    ...
    void crearUsuario_nombreValido() {
        ...
        // Verificación
        assertEquals(userName, user.userName(), "El nombre de usuario no coincide con el esperado");
        assertTrue(user.tweets().isEmpty(), "El usuario recién creado debe tener la lista de tweets vacía");
    }
    ...
    void agregarTweetOriginal() {
        ...
        assertEquals("Este es un tweet original", tweets.get(0).text(), "El texto del tweet no coincide");
    }
    ...
    void agregarRetweet() {
        ...
        // Verificación
        List<Tweet> tweets = user2.tweets();
        assertEquals(1, tweets.size(), "Debe haber un tweet en la lista");
        assertEquals(originalTweet, tweets.get(0).originalTweet(), "El retweet no referencia correctamente al tweet original");
    }
    ...
    void eliminarTweetsUsuario() {
        ...
        // Verificación
        assertTrue(user.tweets().isEmpty(), "La lista de tweets debe estar vacía después de eliminar los tweets");
    }
}
